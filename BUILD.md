...
BSDMC is a portable application and is not supposed to be installed into any system folders.
...
```
git clone git@github.com:Syping/BSDMC.git
git submodule init
...
Getting the project to build and run on FreeBSD is easy if you use any modern and up-to-date FreeBSD distribution.

## Build dependencies
* A C++ compiler capable of building C++11 code.
* Qt 5.6+ Development tools (for example, `qt5`)
* cmake 3.1 or newer (for example, `cmake`)
* zlib (for example, `lzlib`)
* Java JDK 8 (for example, `openjdk8`)
* GL headers (for example, `mesa-libs`)
...
Let's say you want everything in `~/BSDMC/`:

```
# make all the folders
mkdir ~/BSDMC && cd ~/BSDMC
mkdir build
mkdir install
# clone the complete source
git clone --recursive https://github.com/Syping/BSDMC.git src
...
3. Navigate to the BSDMC source folder you cloned and choose CMakeLists.txt.
...
6. Cross your fingers and press the Run button (bottom left of Qt Creator).
    - If the project builds successfully it will run and the BSDMC window will pop up.
...
2. Once that is open, change into your user directory, and clone BSDMC by doing `git clone --recursive https://github.com/Syping/BSDMC.git`, and change directory to the folder you cloned to.
...
5. Run the command `mingw32-make install`, and it should install BSDMC, to whatever the `-DCMAKE_INSTALL_PREFIX` was.
6. In most cases, whenever compiling, the OpenSSL dll's aren't put into the directory to where BSDMC installs, meaning you cannot log in. The best way to fix this is just to do `copy C:\OpenSSL-Win32\*.dll C:\Where\you\installed\BSDMC\to`. This should copy the required OpenSSL dll's to log in.
