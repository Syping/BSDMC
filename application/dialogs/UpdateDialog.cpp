...
void UpdateDialog::loadChangelog()
{
    ...
    if(channel == "stable")
    {
        url = QString("https://raw.githubusercontent.com/Syping/BSDMC/%1/changelog.md").arg(channel);
        m_changelogType = CHANGELOG_MARKDOWN;
    }
    else
    {
        url = QString("https://api.github.com/repos/Syping/BSDMC/compare/%1...%2").arg(BuildConfig.GIT_COMMIT, channel);
...
QString reprocessMarkdown(QByteArray markdown)
{
    ...
    output.replace(QRegExp("GH-([0-9]+)"), "<a href=\"https://github.com/Syping/BSDMC/issues/\\1\">GH-\\1</a>");
    qDebug() << output;
    return output;
}
    ...
    try
    {
        ...
        auto print_commits = [&]()
        {
            ...
            for(int i = commitarray.size() - 1; i >= 0; i--)
            {
                ...
                if(issuenr.length())
                {
                    result += QString("<a href=\"https://github.com/Syping/BSDMC/issues/%1\">GH-%2</a>").arg(issuenr, issuenr);
                }
